...
import { mkdir, open, readdir, readFile, rm, symlink } from "fs/promises";
...
async function init() {
    ...
    if (transportCompanyId) {
        if (transportCompanyId in config.transportCompanies) {
            console.log(
                "List of transport lines for " +
                config.transportCompanies[found[0]] +
                ":"
            );
            await run(transportCompanyId);
        } else {
            var filteredTransportCompanies = filterObject(
                config.transportCompanies,
                (val, key) =>
                    [val, key].some((value) =>
                        value
                            .toLowerCase()
                            .includes(transportCompanyId.toString().toLowerCase())
                    )
            );
            ...
            if (found.length == 1) {
                ...
                console.log(
                    "List of available transport companies:",
                    config.transportCompanies
                );
            }
        }
...
async function run(transportCompanyId) {
    ...
    await ftpDownload(
        ...
        config.ftp.remoteFile,
        config.paths.zip
    );
    await unzip(config.paths.zip, config.paths.unpacked);
    await getTransportLines(
        transportCompanyId,
        config.paths.unpacked,
        config.paths.output
    );
}
                ...
                fd.createReadStream()
                    ...
                    .on("data", async function (row) {
                        if (row[2] == transportCompanyId) {
                            await symlink(
                                outputPath.replaceAll(/[^\/]+/gi, "..") + filePath,
                                outputPath + file
                            );
                            console.log("\t- " + row[1]);
                        }
...
async function unzip(zipFile, outputPath) {
    ...
    fd.createReadStream().pipe(unzipper.Extract({ path: outputPath }));
}
